...
var lastColor = 0;
var colors = ["#3A6BC1","#A5A5A5","#65A1D8","#FFC000","#ED7C30","#63A536","#993E01","#1A3A71"];
...
function convertSelectionToText() {
  var description = (currDivision=="unrestricted" ? "" : "division: " + currDivision + "\n") + 
  (currOwnership =="unrestricted" ? "" : currOwnership + "\n") + 
  (currLength =="unrestricted" ? "" : currLength + "\n") + 
  (currIsR1 ? "R1 only \n" : "") + 
  (currJobType =="unrestricted" ? "" : currJobType + "\n") + 
  (currBEAZones.size == 0 ? "" : ("BEA Zones: " + Array.from(currBEAZones).join(' ') + "\n")) + 
  (currCareerAreas.size == 0 ? "" : ("Career areas: " + Array.from(currCareerAreas).join(' ')));
  if(description == "") {
...
function drawLineGraph(data) {
  ...
  d3.selectAll("#line").each(function(d) {
    descriptions.set(JSON.stringify(d),convertSelectionToText());
  })
  // Add the X Axis
  svg
    .append("g")
...
function addPath(data, svg, valueline) {
  ...
  svg
    ...
    .on("mouseover", function () {
      ...
      d3.select("#description").html(descriptions.get(this.attributes[2].value));
